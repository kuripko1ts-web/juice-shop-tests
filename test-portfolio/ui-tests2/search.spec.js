...
describe('Search Тести - Пошук за назвою продукту', () => {
  test('@search @positive Сценарій Пошук за повною назвою продукту', async ({ page }) => {
    ...
    console.log(`✅ Знайдено ${productCount} продуктів за запитом "Apple Juice"`);
  });

  test('@search @positive Сценарій Пошук за частковою назвою продукту', async ({ page }) => {
    ...
    const productCount = await searchPage.getProductCount();
    expect(productCount).toBeGreaterThan(0);
    console.log(`✅ Знайдено ${productCount} продуктів за частковим запитом "Apple"`);
  });
});

describe('Search Тести - Пошук з невалідними запитами', () => {
  test('@search @negative Сценарій Пошук з неіснуючим продуктом', async ({ page }) => {
    ...
    const noResultsMessage = await page.locator('text=No results').or(
    ...
    console.log(`📋 Повідомлення про відсутність результатів: ${messageVisible}`);
  ...
  test('@search @negative Сценарій Пошук зі спецсимволами', async ({ page }) => {
    ...
    const productCount = await searchPage.getProductCount();
    console.log(`📋 Кількість продуктів після пошуку зі спецсимволами: ${productCount}`);
    console.log('✅ Система успішно обробила спецсимволи без помилок');
  });
});

describe('Search Тести - Пошук з різними форматами', () => {
  test('@search @validation Сценарій Пошук з великими літерами', async ({ page }) => {
    ...
    const productCount = await searchPage.getProductCount();
    expect(productCount).toBeGreaterThan(0);
    console.log(`✅ Пошук case-insensitive: знайдено ${productCount} продуктів`);
  });

  test('@search @validation Сценарій Пошук з малими літерами', async ({ page }) => {
...
describe('Search Тести - Валідація елементів пошуку', () => {
  test('@search @validation Сценарій Перевірка наявності кнопки пошуку', async ({ page }) => {
    ...
    const isSearchButtonVisible = await searchPage.isSearchToggleButtonVisible();
    expect(isSearchButtonVisible).toBeTruthy();
    console.log('✅ Кнопка пошуку видима на сторінці');
  });

  test('@search @validation Сценарій Перевірка відкриття поля пошуку', async ({ page }) => {
...
describe('Search Тести - Пошук після логіну', () => {
  test('@search @auth Сценарій Пошук продукту після логіну', async ({ page }) => {
