  ...
  try {
    ...
    if (isVisible) {
      ...
      for (let i = 0; i < allInputs.length; i++) {
        ...
        console.log(
          `   - Input ${i}: type="${type}", id="${id}", placeholder="${placeholder}", class="${className}", visible=${visible}`
        );
      }
      ...
      for (let i = 0; i < Math.min(10, allButtons.length); i++) {
        ...
        console.log(
          `   - Button ${i}: text="${text.trim()}", class="${className}", visible=${visible}`
        );
      }
    ...
    if (productCards.length > 0) {
      console.log(
        `   - Знайдено ${productCards.length} product-card елементів`
      );
    } else {
    ...
    console.log(
      `   - Знайдено ${products.length} елементів з "product" в класі`
    );
